...
Inbox pattern, dedup, retry, status lifecycle. Shipped by change 01 (M1.3), in `agent_orchestrator`.
...
Inbox rows SHALL move `pending → processing → processed | skipped`, or on error back to `pending` with `retry_count` incremented and exponential backoff; after 3 failed attempts the row becomes `failed` and an admin alert is sent. Failed rows SHALL be re-queueable manually. An early-warning admin alert SHALL fire as soon as failures cross a threshold, distinct from and earlier than the terminal "stuck" alert.
