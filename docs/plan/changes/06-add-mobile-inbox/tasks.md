...
- [ ] 6.4 Production acceptance: from an enrolled device, sign in → locate a failing worker → inspect a customer detail → requeue a failed row and cancel an approved unsent row. Confirm the console is unavailable when secrets are absent, inaccessible outside the tailnet, `/health` still works for Docker, and no list/log exposes message content. — **Open: needs a live run.** Procedure is written up in `production-acceptance-drill.md` (Section A); it cannot be automated — it requires a real enrolled device.
