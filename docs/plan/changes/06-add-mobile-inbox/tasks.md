...
- [ ] 6.4 **Deferred — no phone-reachable console to run it against (R53). Important: do not close change 06 as shipped without this.** Production acceptance: from an enrolled device, sign in → locate a failing worker → inspect a customer detail → requeue a failed row and cancel an approved unsent row. Confirm the console is unavailable when secrets are absent, inaccessible outside the tailnet, `/health` still works for Docker, and no list/log exposes message content.
  ...
  - **Scope of the gap — remote access only.** The console works from the host today at `http://localhost:3100/console/` (`localhost` is a browser secure context, so the `Secure` cookie is honoured). A phone cannot substitute the LAN IP: `http://192.168.88.25:3100` is not a secure context, so the cookie is dropped and the session cannot persist (`console-session.ts:87`). Remote use needs a TLS terminator — Tailscale Serve per `operations.md`, or the nginx already on `:443`. That is a config choice on this same host, not a migration; it rewrites this drill's § A7 and the "Tailnet transport" entry in `design.md`.
  - Status today: bound to `127.0.0.1:3100` — host-only, the intended posture until a terminator is chosen.
