...
### Requirement: Per-provider usage accounting
Every LLM call SHALL record provider, model, role, token counts, and computed cost to the cost-tracking table (whatsapp_manager `api_costs` pattern), enabling per-customer/per-day cost reporting.
