...
| R53 | `/console` is reachable from the host only — there is no path to it from a phone. Nothing needs to move: it runs on the dev host, bound to `127.0.0.1:3100` (`main.ts:685`), and **from a browser on that host `http://localhost:3100/console/` works today** (`localhost` is a browser secure context, so the `Secure` cookie is honoured). The gap is remote access: Tailscale is not installed (no binary/daemon/`tailscale0`/`100.64/10`), so the tailnet gate and MagicDNS name that `operations.md` and `design.md` § "Threat-model checklist" both presume **do not exist**. A phone cannot use a LAN IP instead — `192.168.88.25` is not a secure context, so the `Secure` cookie (`console-session.ts:87`) is dropped and the session cannot persist. Remote access therefore needs a TLS terminator in front (Tailscale Serve, or the nginx already on `:443`), which is a config choice on this same host, not a migration. Blocks change 06 task 6.4 and § B of `production-acceptance-drill.md`. **Not blocking day-to-day console use from the host.** | using the console from a phone / closing change 06 | 🔴 |
