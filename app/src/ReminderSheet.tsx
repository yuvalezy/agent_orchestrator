...
import { BellPlus, Check, Loader2, Plus, Trash2, X } from 'lucide-react';
...
/**
 * Self-contained reminders surface: a Bell trigger button (dropped beside the settings gear in
 * ScreenHeader) that opens a bottom sheet mirroring SettingsSheet/DetailSheet. The sheet holds a
 * "New reminder" form (free text + a native datetime picker anchored at now) that posts a bare
 * wall-clock the server reads in the founder tz, plus a live list of upcoming PENDING reminders
 ...
 */
export function ReminderSheet(): ReactElement {
  ...
  return (
    <>
      <button
        ...
        className="grid size-10 shrink-0 place-items-center rounded-full text-zinc-400 active:bg-zinc-800"
      >
        <BellPlus size={20} />
      </button>

      <div className={cn('fixed inset-0 z-40 transition-opacity', open ? 'opacity-100' : 'pointer-events-none opacity-0')}>
        <button aria-label="Close reminders" onClick={() => setOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
        <div
          role="dialog"
          aria-modal="true"
          aria-label="Reminders"
        ...
        >
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <button aria-label="Close" onClick={() => setOpen(false)} className="grid size-9 place-items-center rounded-full text-zinc-400 active:bg-zinc-800">
              <X size={20} />
            </button>
          </div>
