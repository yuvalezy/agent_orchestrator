...
function InboxView(): ReactElement {
  const [status, setStatus] = useState(''); const [searchInput, setSearchInput] = useState(''); const [search, setSearch] = useState(''); const [selected, setSelected] = useState<string | null>(null); const [cursor, setCursor] = useState<string | null>(null); const [previousCursors, setPreviousCursors] = useState<Array<string | null>>([]); const [requeueCandidate, setRequeueCandidate] = useState<string | null>(null);
  const inbox = useQuery({ queryKey: ['inbox', status, search, cursor], queryFn: () => api<Page>(`/inbox?${new URLSearchParams({ limit: '25', ...(status ? { status } : {}), ...(search ? { search } : {}), ...(cursor ? { cursor } : {}) })}`) });
  const detail = useQuery({ queryKey: ['inbox', selected], queryFn: () => api<{ data: Row }>(`/inbox/${selected}`), enabled: Boolean(selected) });
  const client = useQueryClient();
  const requeue = useMutation({ mutationFn: (id: string) => api(`/inbox/${id}/requeue`, { method: 'POST' }), onSuccess: () => { void client.invalidateQueries({ queryKey: ['inbox'] }); setRequeueCandidate(null); setSelected(null); } });
...
function DecisionsView(): ReactElement {
  const [selected, setSelected] = useState<string | null>(null); const [type, setType] = useState(''); const [outcome, setOutcome] = useState(''); const [searchInput, setSearchInput] = useState(''); const [search, setSearch] = useState(''); const [cursor, setCursor] = useState<string | null>(null); const [previousCursors, setPreviousCursors] = useState<Array<string | null>>([]);
  const decisions = useQuery({ queryKey: ['decisions', type, outcome, search, cursor], queryFn: () => api<Page>(`/decisions?${new URLSearchParams({ limit: '25', ...(type ? { type } : {}), ...(outcome ? { outcome } : {}), ...(search ? { search } : {}), ...(cursor ? { cursor } : {}) })}`) });
