    ...
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-5 text-sm leading-6 text-zinc-400">
        ...
        <ol className="mt-2 list-decimal space-y-1 pl-5">
          <li>Search the portal for the customer. Already-onboarded ones are marked and can't be picked again — manage an existing customer's <span className="text-zinc-300">modules</span> from the Customers screen.</li>
      ...
      {selected && (
        <Panel step={2} title="Contacts we'll import">
          ...
          {preview.data && (
            <>
              {preview.data.data.alreadyOnboarded && <ErrorText message="This customer is already onboarded — pick another. (Edit its modules from the Customers screen.)" />}
      ...
      {/* Step 5: backfill */}
      {onboardedId && <BackfillPanel step={5} customerId={onboardedId} confirmLive={confirmLive} setConfirmLive={setConfirmLive} />}
    </section>
  );
}
