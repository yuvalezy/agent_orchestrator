...
//
// A stand-in core-domain module: it lives in the REAL core dir `src/inbox/` and
// ILLEGALLY imports an adapter. Because it sits inside a real core dir, the MAIN
// eslint.config.mjs D1 zone (`src/inbox` → `src/adapters`) is exactly what must
// reject it — so this guard exercises the actual target list the app relies on,
// not a boundary-specific stand-in.
//
