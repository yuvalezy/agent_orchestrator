...
export const SCHEDULE_SCHEMA = {
  ...
  required: ['kind', 'execute_at', 'explicit_date', 'body', 'meeting_topic', 'delivery_channel', 'clarification', 'recurrence', 'attendees', 'duration_minutes'],
  properties: {
    ...
    meeting_topic: { type: ['string', 'null'] },
...
export const SCHEDULE_SYSTEM = [
  ...
  '- meeting_topic: a concise attendee-visible NOUN PHRASE for the PURPOSE the founder explicitly',
  '  stated. Use only founder-authoritative commandText/priorCommandText; never infer a purpose from',
  '  the customer name, attendees, repliedText, mappedOutboundBody, or general context. Omit dates,',
  '  times, attendee names, and scheduling verbs such as "call", "meeting", "schedule", or "book".',
  '  Keep it specific and short (at most 8 words / 80 characters). Examples:',
  '    "meet Dana about renewal pricing" -> "Renewal pricing"',
  '    "review onboarding blockers with Idan" -> "Onboarding blockers"',
  '    "set up a call with Dana tomorrow" -> null (no purpose was stated; code supplies a fallback).',
  '  A refinement that only changes time/attendees returns null so the prior topic is preserved.',
  '- body: null. A meeting is not a customer message; its title lives only in meeting_topic.',
  'For every other kind set attendees=null and duration_minutes=null.',
  'For every non-meeting kind set meeting_topic=null.',
  '',
  ...
  '',
  'For kind=customer_message, body is what the CUSTOMER will read. It is NEVER your instructions',
  'from the founder. These body-writing rules do not apply to meetings (their body stays null):',
