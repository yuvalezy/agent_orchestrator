...
test('the FCM message is DATA-ONLY (the PWA SW renders it; a notification block would double up)', () => {
  const msg = buildMulticastMessage(['tok-1', 'tok-2'], { messageId: 'm-1', kind: 'question', severity: 'warning', ref: 'task-42', route: '/app/customer/cust-9' });
  ...
  assert.deepEqual(msg.webpush, { headers: { Topic: 'task-42' }, fcmOptions: { link: '/app/customer/cust-9' } });
});
